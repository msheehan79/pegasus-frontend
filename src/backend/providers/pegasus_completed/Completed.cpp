...
{
    return paths::writableConfigDir() + QStringLiteral("/completed.txt");
}
} // namespace
...
Completed::Completed(QObject* parent)
    : Completed(default_db_path(), parent)
{}

Completed::Completed(QString db_path, QObject* parent)
    : Provider(QLatin1String("pegasus_completed"), QStringLiteral("Pegasus Completed"), PROVIDER_FLAG_INTERNAL | PROVIDER_FLAG_HIDE_PROGRESS, parent)
    , m_db_path(std::move(db_path))
{}

Provider& Completed::run(SearchContext& sctx)
{
    ...
    if (!db_file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        Log::error(display_name(), LOGMSG("Could not open `%1` for reading, completed games not loaded").arg(m_db_path));
        return *this;
    }
    ...
    while (db_stream.readLineInto(&line)) {
        ...
        if (game_ptr)
            game_ptr->setCompleted(true);
    }

    return *this;
}

void Completed::onGameCompletedChanged(const std::vector<model::Game*>& game_list)
{
    const QMutexLocker lock(&m_task_guard);
    const QDir config_dir(paths::writableConfigDir());

    m_pending_task.clear();
    m_pending_task << QStringLiteral("# List of completed games, one path per line");
...
void Completed::start_processing()
{
    ...
    QtConcurrent::run([this]{
        ...
        while (true) {
            ...
            if (!db_file.open(QIODevice::WriteOnly | QIODevice::Text)) {
                Log::error(display_name(), LOGMSG("Could not open `%1` for writing, completed games are not saved")
                    .arg(m_db_path));
                break;
            }
            ...
            for (const QString& cpl : qAsConst(m_active_task))
                db_stream << cpl << Qt::endl;
